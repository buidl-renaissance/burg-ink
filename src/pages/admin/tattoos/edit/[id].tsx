...
export default function EditTattooPage() {
  ...
  const fetchTattoo = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to fetch tattoo');
      }
      const data = await response.json();
      setTattoo(data);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to load tattoo');
    } finally {
      setLoading(false);
    }
  };
  ...
  const statusBarContent = (
    <StatusBarInner>
      ...
      <SubmitButton onClick={handleSubmit} disabled={isSubmitting}>
        <FaCheck /> {isSubmitting ? 'Updating...' : 'Update Tattoo'}
      </SubmitButton>
    </StatusBarInner>
  );
  ...
  if (loading) {
    return (
      <AdminLayout currentPage="tattoos" statusBar={statusBarContent}>
        <Container>
          <LoadingContainer>
    ...
    return (
      <AdminLayout currentPage="tattoos" statusBar={statusBarContent}>
        <Container>
          <ErrorContainer>
            <h2>Error</h2>
            <p>{error || 'Tattoo not found'}</p>
            <BackButton onClick={handleCancel}>
              Back to Tattoos List
            </BackButton>
          </ErrorContainer>
        </Container>
      </AdminLayout>
